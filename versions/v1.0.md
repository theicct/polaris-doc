...
<!-- Added by: gabealvarez, at: Fri Mar  3 13:05:44 PST 2023 -->
...
$$\text{NewBuildStock}_{\text{ship}\text{_}class,B\text{in}} = \frac{\text{ActivityNeeded}_{\text{ShipClass}}*\text{ShareOfActivity}_{ShipClass \rightarrow ship\text{_}class,B\text{in}}}{\text{MeanActivityOfVessel}_{\text{ship}\text{_}class,B\text{in}}}$$
...
$$Required\ EEDI = \left( 1 - \frac{X}{100} \right)*\ EEDI\ \text{re}f\text{erence}\ \text{line}\ \text{value}$$
...
$$\text{MaxSpee}d' = MaxSpeed*\left( 1 - E\text{EPL}_{\text{EEDI}} \right)^{1/3}$$
...
The EEDI EPL may be arbitrarily close to zero but is not allowed to exceed 90%. New-build ships which cannot be made to comply with EEDI via this scheme are flagged for the user and built to be as close to compliance as possible.
...
$$\text{Require}d\ E\text{EXI} = \left( 1 - \frac{Y}{100} \right)*\ EEDI\ reference\ line\ value$$
...
The EEXI EPL is also not allowed to exceed 90%. Existing ships which cannot be made to comply with EEXI via this scheme are flagged for the user and retired.
...
Note that we do not currently enforce a minimum power requirement; the only guardrail is the upper bound on EEXI EPL (90%).
...
$$\text{Activit}y^{'} = \ Activity*\ \frac{\text{Distance}\ traveled'}{\text{Distance}\ \text{traveled}}$$
...
Similar to average operational speed, the average power consumed by the main engine in cruise phase is calculated as a sum over main engine power load factors (\\(\text{Lf}\\)):

$$\ \ \text{MeanPowerConsume}d_{\text{Main}}\ \left\lbrack kW_{\text{in}} \right\rbrack = \ MaxPowerOutput_{\text{Main}}\ \left\lbrack kW_{\text{out}} \right\rbrack*\sum_{\text{Lf}}^{}{\left( Lf*\text{ShareOfHours}_{\text{Lf}}*\ \frac{1}{\text{EngineEfficienc}y_{\text{Lf}}}\left\lbrack \frac{kW_{\text{in}}}{kW_{\text{out}}} \right\rbrack \right)\ }$$

where the engine efficiency is given as

$$\text{EngineEfficiency}_{\text{Lf}}\left\lbrack \frac{kW_{\text{out}}}{kW_{\text{in}}} \right\rbrack\  = \frac{1}{\text{SpecificFuelConsumptio}n_{\text{Lf}}\left\lbrack g/kWh_{\text{out}} \right\rbrack\ *\ FuelEnergyContent\ \lbrack\text{kW}h_{\text{in}}/g\rbrack\ }$$

and the specific fuel consumption varies with load factor according to the relationship established in the [Third IMO GHG Study 2014](https://wwwcdn.imo.org/localresources/en/OurWork/Environment/Documents/Third%20Greenhouse%20Gas%20Study/GHG3%20Executive%20Summary%20and%20Report.pdf):

$$\text{SpecificFuelConsumptio}n_{\text{Lf}} = SpecificFuelConsumption_{\text{Baseline}}*(0.455*Lf^{2} - 0.71*Lf + \ 1.28)$$

The total energy consumption by the main engine in cruise phase is then determined by:

$$\text{EnergyConsume}d_{\text{Main}}\  = MeanPowerConsumed_{\text{Main}}*CruiseHours$$

where the estimate for cruise hours has already been limited by the historical maximum after any EPLs are applied.
...
<p>LNG and Bio-LNG - <a href="https://theicct.org/wp-content/uploads/2022/09/Renewable-LNG-Europe_report_FINAL.pdf">Comer et al. (2022)</a></p>
